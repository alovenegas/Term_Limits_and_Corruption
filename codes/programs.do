...
gen term = 1
* Allowed
replace term = 0 if inlist(municipality,"OROTINA","FLORES","CARTAGO","SAN PABLO","CURRIDABAT","SANTO DOMINGO","PALMARES","BARVA")
replace term = 0 if inlist(municipality,"GRECIA","GOICOECHEA","LA UNION","ALAJUELA","POAS","TIBAS","OREAMUNO")
replace term = 0 if inlist(municipality,"SARCHI","PARAISO","SANTA BARBARA","MORA","PURISCAL","SANTA CRUZ","POCOCI","LIBERIA","NICOYA")
replace term = 0 if inlist(municipality,"BAGACES","RIO CUARTO","PUNTARENAS","QUEPOS","ABANGARES","DOTA","COTO BRUS")
...
cap drop reelec
gen reelec = 1
replace reelec = 0 if inlist(municipality,"HEREDIA","ABANGARES","LA UNION","PALMARES","PARAISO","PURISCAL","RIO CUARTO","SARAPIQUI")
...
replace reelec = 0 if inlist(municipality,"GOLFITO","FLORES","POAS","OREAMUNO","SANTA BARBARA","UPALA","ALAJUELA","SANTO DOMINGO","")
...
program define q_event_study 
	syntax [, outcome(varlist) controls(varlist) indepvars(varlist)]
...
coefplot (matrix(b), v(V)), ///
         ...
         graphregion(color(white)) ytitle(`ylab') xtitle("Quarters from treatment")
